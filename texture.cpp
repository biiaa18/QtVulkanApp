...
Texture::Texture()
{
    ...
    makeTexture(); //will make simple colors, not use a premade texture file
}
...
bool Texture::readBitmap(const string &filename)
{
    ...
    ifstream file;
    file.open (fileWithPath.c_str(), ifstream::in | ifstream::binary);
    if (file.is_open())
    {
